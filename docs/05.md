...
The handler function signature, which is assumed by the `HANDLE_WM_SIZE` macro, is documented in a comment along with the defininition of that macro in Microsoft’s (but unfortunately not in the MinGW) `<windowsx.h>` header.

For completeness, the complete dialog window message handler, its “dialog proc”, which invokes `WM_HANDLE_SIZE` via an internal [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself)-support macro `HANDLE_WM`:
