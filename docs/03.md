...
In modern C++ terms this indicates a dialog proc result type like `struct{ bool processed; optional<int> msg_result; }`, but a Windows dialog proc has a single integer return value…

Our current (version 3.3.1) dialog proc works and *can* work because it only processes two messages, namely `WM_CLOSE` which doesn’t have a message processing result, and `WM_INITDIALOG` which is only used with dialogs and fits the single integer result:
...
| Message                | Dialog proc return value communicates:                 |
| ---------------------- | ------------------------------------------------------ |
...
| *Any other message*    | *Whether the message was processed or not.*            |
