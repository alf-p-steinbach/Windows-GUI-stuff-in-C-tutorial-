...
#### ***3.3.3.*** Reuse support machinery in self contained headers.
...
We’ll start with a sort of minimum, *one* function, namely `contains` function, which goes into a header **`"cpp_support_machinery.hpp"`** (the underscores in the header name reflect that this is name in the C++ code in the header, otherwise I’d instead use hyphens):
...
In passing, one way to check that the header is actually **self contained** — that it itself includes everything that it needs — is to compile with a *forced include* of the header, which is Visual C++ option `/FI` and g++ option `-include`:
