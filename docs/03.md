...
* we’ll prepare the dialog proc for handling of general Windows messages by value by using the undocumented `SetDlgMsgResult` macro (*most* messages require this!);
...
That’ because it defines lower case macros like `min` and `max` that Really Foul Up™ things, and not just `std::min` and `std::max`. To make things worse at least one of Microsoft’s own C++ specific headers, namely `<gdiplus.h>` when it’s included from C++, *depends* on these macros. But when the choice is between avoiding using the C++ standard library, and avoiding using `<gdiplus.h>` in its raw counter-productive form, it’s a really easy choice.

There are two main approaches to get rid of an undesired `<windows.h>` macro:

* Ask `<windows.h>` to never define it in the first place.  
...
In *[03/code/tic-tac-toe/v3/v3-1/main.cpp](03/code/tic-tac-toe/v3/v3-1/main.cpp)*:
~~~ cpp
...
[*03/code/tic-tac-toe/v3/v3-1/wrapped-windows-h.hpp*](03/code/tic-tac-toe/v3/wrapped-windows-h.hpp)
~~~cpp
#pragma once
#ifdef UNICODE
...
---
#### ***3.3.2.*** Prepare the dialog proc for handling of general Windows messages.

For the processing of most Windows messages a dialog proc has to communicate two pieces of information back to the caller:
...
However, the dialog proc return value generally only supports the “was it processed” boolean value.

This is sufficient when a message doesn’t *have* a processing result, such as for `WM_CLOSE`. It’s also sufficient for a handful of special *dialog messages* such as `WM_INITDIALOG`. And this is the reason why the current (version 3.3.1) dialog proc works and *can* work:
...
The special dialog messages are:

* Supported by the `SetDlgMsgResult` macro:  
  **`WM_CTLCOLORMSGBOX`**, **`WM_CTLCOLOREDIT`**, **`WM_CTLCOLORLISTBOX`**, **`WM_CTLCOLORBTN`**, **`WM_CTLCOLORDLG`**, **`WM_CTLCOLORSCROLLBAR`**, **`WM_CTLCOLORSTATIC`**, **`WM_COMPAREITEM`**, **`WM_VKEYTOITEM`**, **`WM_CHARTOITEM`**, **`WM_QUERYDRAGICON`** and **`WM_INITDIALOG`**.
...
#### **3.3.x. Factor out reusable support functionality.**
