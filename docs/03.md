...
Version 2’s general reusable **wrapper function** `set_icon` that provides clarity and convenience should not go into the `<windows.h>` wrapper, but into a separate header that can provide more such support machinery. More generally a header wrapper should not provide additional functionality; just a clean include, which minimizes name pollution. That makes a wrapper such as `"wrapped-windows-h.hpp"` reliable and easy to understand, where one knows roughly what it does just from the filename.
