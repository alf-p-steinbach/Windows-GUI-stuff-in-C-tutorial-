...
We’ll start with a sort of minimum refactoring challenge, just *one* function, namely the `contains` function. It goes into a header `"reuse/cpp_support_machinery/`**`value-constraints-checking.hpp"`**. Since the resource handling code validates that an integer id is in a given range, a function for that, called `is_in`, is also added here.
