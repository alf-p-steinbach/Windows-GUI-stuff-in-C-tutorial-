...
Direct use of the can look like this:
...
To reduce the verbosity of the message cracker calls and to correspondingly increase the  of the code, this program’s `"windows-support-machinery.hpp"` header defines the helper macro
...
But the Windows API does not support C++ `optional`. The connection is made by the dialog proc (the callback for the API level), which inspects the `optional` and calls the `<windowsx.h>` macro **`SetDlgMsgResult`** if there is a return value. The `SetDlgMsgResult` macro, in turn, checks what the current message is, and returns the return value in appropriate fashion depending on the message (it’s a mystery to me why Microsoft made this so needlessly complex, but presumably some decades ago there was some apparently important reason):
