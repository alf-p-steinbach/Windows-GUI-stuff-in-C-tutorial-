...
Next, how to find the processing function signatures expected by the message crackers. As far as I know these are only available ***as comments*** in Microsoft’s `<windowsx.h>`, but e.g. not in the g++ version of that header. Happily versions with the signature comments is available freely on the nets, e.g. in the Wine project, and in project [at Github](https://github.com/tpn/winsdk-10/blob/master/Include/10.0.10240.0/shared/windowsx.h), which I’ll use for the example here.
