...
This is sufficient when a message doesn’t *have* a processing result, such as for `WM_CLOSE`. It’s also sufficient for a handful of special **dialog messages** such as `WM_INITDIALOG`. And since only those two messages are processed with current (version 3.3.1) dialog proc, it works and *can* work:
