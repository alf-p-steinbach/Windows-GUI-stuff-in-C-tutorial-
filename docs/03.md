...
First, to exclude some of the irrelevant and nasty sabotage-like stuff from `<windows.h>` one can define various macros, such as the two below, before including that header:
...
In addition one should ensure that `STRICT` is defined, even though that now appears to be the default with all compilers. **`STRICT`** ensures C++ compatible fully typed declarations. In the early Windows days one mainly used C for Windows programming and `<windows.h>` supported that by default, where one had to define `STRICT` to get everything to compile as C++.

With Visual C++ 2020 `WIN32_LEAN_AND_MEAN` reduces the number of lines from 303 818 to 161 309, quite significant; it avoids defining the `small` macro by not including `<rpcndr.h>`; and it avoids including the `<winsock.h>` header which can have the same include guard as (and hence prevents inclusion of) the `<winsock2.h>` header that one really wants in modern code. So, `WIN32_LEAN_AND_MEAN` should really be the modern default, much as `STRICT` is now the default. Alas.
...
… is therefore now replaced with `#include` of a reusable **header wrapper** that defines the above two mentioned macros, and more,
...
There’s a lot more that can and maybe should go into a `<windows.h>` wrapper, like sometimes crucial “settings” of `<windows.h>` such as the `WIN32_WINNT` version macro, but the above is good enough for our purposes.

<p align="center">❁ &nbsp; ❁ &nbsp; ❁</p>

In this version of the Tic Tac Toe program we’ll also use some convenience macros from **`<windowsx.h>`**, and unfortunately like many Microsoft headers `<windowsx.h>`

* depends on `<windows.h>` but
* is not guaranteed to include `<windows.h>` itself.

I.e. `<windowsx.h>` is not a best practice guaranteed **self contained header**. Microsoft recommends including `<windows.h>` before any of their other headers, but best practice for just about everyone else is that the order of header inclusion should not matter. So instead of using `<windowsx.h>` directly we’ll use a self-contained header wrapper:
...
To make it reliable and easy to understand, where one knows roughly what it does just from the filename, and to minimize name pollution, a header wrapper should not provide additional functionality; just a clean include. Version 2’s general reusable **wrapper function** `set_icon` that provides clarity and convenience should therefore not go into the `<windows.h>` wrapper, but into a separate header that can provide more such support machinery.
...
&hellip; expands to a much less comprehensible call of `my_initdialog_handler` like
...
&hellip; where essentially the message parameters extraction is nothing more than a cast to `HWND` of the `WPARAM` message parameter.

All these macros can be used in exactly the same way because every message cracker macro takes the same arguments and produces an `LRESULT` return value, even when your message handler is a `void` function.

To reduce the verbosity of the message cracker calls and to correspondingly increase the [DRY-ness](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself) of the code, this program’s `"windows-support-machinery.hpp"` header defines the helper macro

~~~cpp
#define WSM_HANDLE_WM( name, handler_func ) \
    WSM_HANDLE_WM_##name( msg.hwnd, msg.wParam, msg.lParam, handler_func )
~~~

&hellip; which just *assumes* that the calling function has the message data conveniently packed in a variable called `msg`, of Windows API type `MSG`, and expands to a call of the specified message cracker macro.

Usage in this version’s main program:
