...
The dialog resource is mainly identified via the `IDD_MAIN_WINDOW` macro defined by the `"resources.h"` header file. It expands to a number that — together with the expected resource type and a specification of which executable the resource is in, the `this_executable` — yields a unique identification. In Microsoft code the `this_executable` is often called the `hInstance`; anyway it can be just a nullvalue for getting the default, but I chose to show it explicitly.

Windows’s **`MAKEINTRESOURCE`** macro casts the `IDD_MAIN_WINDOW` number to a pointer. I named it `resource_id_as_pseudo_ptr` because it’s not a valid C++ pointer value. The resource functions recognize it as an id rather than as a pointer to text string because all bits are zero except the lower 16, and this scheme limits ***the id number range*** to 0 … 32767, inclusive.
...
First, to exclude some of the irrelevant and nasty sabotage-like stuff from `<windows.h>` one can define various macros, such as **`NOMINMAX`** and **`WIN32_LEAN_AND_MEAN`**, before including that header:

* using `NOMINMAX` one avoids getting the nasty-macros `min` and `max`, which like nasty-macro `small` can wreak havoc; and
* using `WIN32_LEAN_AND_MEAN` one cuts the number of `<windows.h>` source lines roughly in half, avoids getting the `small` macro, and removes a problem with Windows’ socket library.

Particulars: with Visual C++ 2020 `WIN32_LEAN_AND_MEAN` reduces the number of lines from 303 818 to 161 309, quite significant. And with this compiler `WIN32_LEAN_AND_MEAN` avoids defining the `small` macro by not including `<rpcndr.h>`. And it avoids including the `<winsock.h>` header which can have the same include guard as (and hence prevents inclusion of) the `<winsock2.h>` header that one really wants in modern code. `WIN32_LEAN_AND_MEAN` should really be the default. Alas.
...
… is now replaced with `#include` of a reusable **header wrapper** that defines the above two mentioned macros, and more,
...
[*03/code/tic-tac-toe/v3/wrapped-windows-h.hpp*](03/code/tic-tac-toe/v3/wrapped-windows-h.hpp)
~~~cpp
#pragma once
#ifdef UNICODE
...
In this version of the Tic Tac Toe program we’ll also use some convenience macros from **`<windowsx.h>`**, and unfortunately like many Microsoft headers it

* depends on `<windows.h>` but
* is not guaranteed to include it itself.

I.e. `<windowsx.h>` is not a guaranteed self contained header. Microsoft recommends including `<windows.h>` before any of their other headers, but best practice for just about everyone else is that the order of header inclusion should not not matter. So instead of using `<windowsx.h>` directly we’ll use a self-contained header wrapper:
...
To make it reliable and easy to understand, where one knows roughly what it does just from the filename, and to minimize name pollution, it should not provide additional functionality; just a clean include. Version 2’s general reusable **wrapper function** `set_icon` that provides clarity and convenience should therefore not go into the `<windows.h>` wrapper, but into a separate header that can provide more such support machinery.

In that Windows API support machinery header it’s natural to include a little class for resource id’s, so that one avoids peppering the code with `MAKEINTRESOURCE` invocations and so that one can overload on resource id as argument, and so that one can have some confidence that an integer actually is a resource id. Let’s call that class `Resource_id`. With that in hand it’s also natural (to me) to add a reusable overload of `set_icon` that sets both the small and large icon with a given resource id, as we’ve found a need for:
...
Here we’ll just avail us of the `HANDLE_VM_CLOSE` and `HANDLE_WM_INITDIALOG` macros, that are examples of **message cracker** macros.

They’re called “cracker” macros because they extract message specific parameters from the `WPARAM` and `LPARAM` values. Each such macro passes the extracted values to a specified **message handler** function, that must have corresponding parameters. For example, modulo that compiler’s multi-line formatting, with the MinGW g++ compiler the message cracking

~~~cpp
HANDLE_WM_INITDIALOG( window, w_param, ell_param, my_initdialog_handler )
~~~

&hellip; expands to a call of `my_initdialog_handler` like

~~~cpp
(LRESULT)(DWORD)(UINT)(WINBOOL) (my_initdialog_handler)(
    (window), (HWND)(w_param), ell_param
    )
~~~

&hellip; where essentially the value extraction is nothing more than a cast to `HWND` of the `WPARAM` message parameter.

All these macros can be used in exactly the same way because every message cracker macro takes the same arguments and produces an `LRESULT` return value, even when the message handler is a `void` function.
...
// E.g., `HANDLE_WM( CLOSE, on_wm::close )` expands to `HANDLE_WM_CLOSE( ...`.
...
{
    switch( msg.message ) {
        case WM_CLOSE:          return HANDLE_WM( CLOSE, on_wm::close );
        case WM_INITDIALOG:     return HANDLE_WM( INITDIALOG, on_wm::initdialog );
    }
    return {};
}
~~~
...
//      <winapi_support_machinery.hpp> introduced as place for simple Windows API utility stuff.
...
namespace wu = winapi_support_machinery;
