...
In modern C++ terms this is a dialog proc result type like `struct{ bool processed; optional<int> msg_result; }`. But Windows’s dialog support was designed with a single integer as dialog proc result. And so the dialog proc result value *generally* only supports the “was it processed” boolean value.

A single integer is sufficient when a message doesn’t *have* a processing result, such as for `WM_CLOSE`. It’s also sufficient for a handful of special **dialog messages** such as `WM_INITDIALOG`. And since only those two messages are processed with the current (version 3.3.1) dialog proc, it works and *can* work:
...
Special dialog messages where simple code like above suffices:
