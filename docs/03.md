...
Our current `WM_INITIDIALOG` handler does not take any message parameters,

```cpp
auto on_wm_initdialog( const HWND window ) -> bool
```
...
If a message processing function is `void` so that it doesn’t *have* a result, then its message cracker macro returns integer 0, which is consistent with our definition of `dialog_message::processed`. So, for example, the result of the `HANDLE_WM_CLOSE` can be returned in just the same way as the result of `HANDLE_WM_INITDIALOG`, even though the former’s processing function is `void`. Nice.
