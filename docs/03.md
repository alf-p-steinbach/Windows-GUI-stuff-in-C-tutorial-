...
An alternative way to make the `dlgproc` reusable could be to make make it depend on a declaration of specially named message processing function, which the client code would then have to provide a definition of. However, that’s mainly a C technique, and it’s limited and awkward. So, using the idea of templating:
