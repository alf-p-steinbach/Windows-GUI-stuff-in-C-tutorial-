...
… is therefore now replaced with `#include` of a reusable **header wrapper** `"wrapped-windows-h.hpp"` that defines the above two mentioned macros, and more,
...
Version 2’s general reusable **wrapper function** `set_icon` that provides clarity and convenience should not go into the `<windows.h>` wrapper, or even the `<windowsx.h>` wrapper, but into a separate header that can provide more such support machinery. For generally a header wrapper should not provide additional functionality; it should just provide a clean include, which minimizes name pollution. That makes wrappers such as `"wrapped-windows-h.hpp"` and `"wrapped-windowsx-h.hpp"` reliable and easy to understand, where one knows roughly what they do just from the filenames.
