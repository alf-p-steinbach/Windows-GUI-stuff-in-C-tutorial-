...
In modern C++ terms this is a result type like `struct{ bool processed; optional<int> value; }`. But Windows’s dialog support was designed with a single integer as dialog proc result. And so the dialog proc result value *generally* only supports the “was it processed” boolean value.
